...
final class OuraRing: NSObject, ObservableObject {
    ...
    func startLive() {
        ...
        publish { self.liveActive = true; self.status = "Preparing live…" }
    ...
    private func runLive() async {
        ...
        let baseline = await drainEventsLive(cursor: cursor) { _ in }
        guard baseline.complete else {
            dbg("live: baseline drain failed at cursor \(cursor)")
            publish { self.liveActive = false; self.status = "Live setup timed out - sync first" }
            return
        }
        ...
        publish { self.status = "Live — measuring…" }
    ...
    private func handleLiveEvent(_ ev: DecodedEvent) {
        ...
        var bpmToPublish: Int?
        var plausibleIbis: [Double] = []
        ...
        if let arr = ev.json["hr_bpm"] as? [Any] {
            ...
            bpmToPublish = bpms.last
        }

        if let arr = ev.json["ibi_ms"] as? [Any] {
            plausibleIbis = arr.compactMap { ($0 as? NSNumber)?.doubleValue }.filter { $0 >= 400 && $0 <= 1300 }
            if bpmToPublish == nil, let ibi = plausibleIbis.last {
                let bpm = Int((60_000.0 / ibi).rounded())
                if bpm > 30 && bpm < 240 { bpmToPublish = bpm }
            }
        }

        if let bpm = bpmToPublish {
            publish {
                self.liveHR = bpm
        ...
        if !plausibleIbis.isEmpty {
            bleQueue.async { [weak self] in
                guard let self else { return }
                self.ibiBuffer.append(contentsOf: plausibleIbis)
